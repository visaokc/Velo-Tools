...
**Velo Compatibility -> Slot-style textures** is optional and off by default. With the default INI template, it consumes fresh schema-v4 `ShaderTextureUsage.json` evidence from the same object source and replaces covered texture Hash overrides with Component-local `ps-tN` bindings. The generated draw transaction backs up every assigned slot, runs EFMI's normal texture-override stage, tags each observed concrete DXGI format while using a shared format-family value in the branch condition, binds the exported resource, and restores the original slots after the Component draw. It does not rely on a `*_TYPELESS` matcher to stand in for typed EFMI resources. Slots are derived from the dump instead of being limited to a fixed range; current EFMI captures have been verified with `ps-t0`, `ps-t1`, and `ps-t11..22`.
