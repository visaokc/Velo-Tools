...
Extraction names, original formats, and diffuse co-occurrence narrow candidates; DDS-header hints are used only when extraction format evidence is absent. Unique suggestions apply automatically. Format alone is not proof of semantic meaning; genuinely ambiguous roles remain unassigned for direct editing. Old saved mappings remain usable without confirmation. Initialization also works without an extraction configured.
