...
Standard full exports now automatically enable the existing RG smooth-normal branch for compatible outline shaders when an exported mesh has nonzero `COLOR.R/G`. This includes generated, painted, and retained imported values; no new checkbox or authoring tag is required. The supported layout is `TEXCOORD.xy` plus `R8G8B8A8_SNORM COLOR` in a 12-byte VB1. Missing/zero RG, CPU-posed components, incompatible layouts, and custom/live templates are left alone. Layout eligibility follows the actual VB1 binding, so an LOD without COLOR stays native without disabling the compatible main mesh. The native selector is extended only during the corresponding custom draw, with draw state restored immediately afterward; native-enabled materials, RG decoding, vertex buffers, and material outline width are not changed. This is not a separate thickness mask or an `R=1` cutout rule: painted-value effects must be checked in game. Regenerate `mod.ini` with a full export, not a buffers-only export. Generate smooth-normal COLOR before manual painting, since running the generator again overwrites it.
