...
Standard full exports automatically enable the existing RG smooth-normal branch for compatible outline shaders when any exported part of a Component has nonzero `COLOR.R/G`. This includes generated, painted, and retained imported values; no new checkbox or authoring tag is required. The entire Component's custom draws then share the activation scope, including parts with zero RG: state is saved once on entry and restored on exit, rather than around every material or visibility draw. LOD layout changes, borrowed Component buffers, original draws, and opaque helpers retain the necessary state boundaries. Supported VB1 layouts contain `R8G8B8A8_SNORM COLOR` after `TEXCOORD.xy`, either at byte 8 in a 12-byte stride or at byte 16 in a 20-byte stride with a second UV/preserved 8-byte field. The structural shader rule covers verified material-layout variants and follows the RG decoder into outline extrusion; it does not assume every shader or future variant is compatible. Missing/zero RG across the entire Component, CPU-posed components, incompatible layouts, and custom/live templates are left alone. An LOD without COLOR stays native without disabling a compatible main mesh. Native-enabled selectors, RG decoding, vertex buffers, and material outline width are unchanged. This is not a separate thickness mask or an `R=1` cutout rule: painted-value effects must be checked in game. Regenerate `mod.ini` with a full export, not a buffers-only export. Generate smooth-normal COLOR before manual painting, since running the generator again overwrites it.
...
1. In Object Mode, select mesh objects and run **Initialize Selected Materials**. Used MMD and Blender materials receive independent semantic shader copies, keeping identifiable diffuse images and UV/vector wiring. Unselected users of shared meshes/materials remain unchanged. Repeating initialization refreshes source mappings on existing assignment materials without rebuilding their nodes or replacing their connections.
...
Grouping does not move parts between Components or across protected objects. Known alpha-blended materials, connected/non-unit preview alpha, CPU-posed Components, positive recorded Endfield transparency flags, and unsplit meshes with mixed texture bindings preserve their placement. Use **Preserve Draw Order** in Material Tools for other order-dependent game passes: Blender preview settings cannot prove that every runtime pass is order-independent. Protected parts remain barriers, so parts on opposite sides are not brought together. Turn **Group Draws by Texture** off to retain the earlier object order and adjacent-only command coalescing. Texture assignment and synchronization still work when grouping is disabled.

Export uses the newly finalized triangle ranges while retaining each draw's conditions. Each group backs up and restores its slots. Components sharing an original can use different replacements. Replacement images are copied into `Textures/` under the exact basename recorded by the connected image path: no prefix, digest suffix, case conversion or extension conversion is added. An existing file with identical bytes is reused without rewriting. An existing same-name file with different bytes, or two inputs whose names collide case-insensitively on Windows, aborts the export rather than overwriting either file; rename the source image or use a clean output folder. Files produced by older builds, including `material_<digest>.*`, are not deleted automatically. This is Slot routing, not new runtime Hash-based per-draw matching. Removed or explicitly disabled original roles are not overridden; unresolved connected roles and unsafe runtime slot assignments still abort instead of guessing.
...
Propagation and other staged material edits preserve the exact active material name whenever one replacement takes over an unused local original. The retained original is labeled `Backup <original name>` instead of forcing the active material to acquire `.001`, `.002`, and so on. Intentional existing suffixes are part of the name and are not stripped. A material that still belongs to another user keeps its name and contents; an actual isolated fork must have a distinct Blender name, which stays stable on subsequent edits. Name transfers participate in the same rollback as slots and sync membership.
