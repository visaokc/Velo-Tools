...
Export preserves finalized triangle ranges, draw order and conditions, and groups only adjacent compatible bindings within a Component. Each group backs up and restores its slots. Components sharing an original can use different replacements, while identical replacement file contents share output resources. This is Slot routing, not new runtime Hash-based per-draw matching. Removed or explicitly disabled original roles are not overridden; unresolved connected roles and unsafe runtime slot assignments still abort instead of guessing.
