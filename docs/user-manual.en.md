...
- **Automatically skip LOD components** is enabled by default. Before unified-VG analysis or any JSON/file output, Velo follows original draw order and compares each possible lower-detail draw only with a small forward neighborhood, then selects that neighborhood's highest voxel-similarity higher-face candidate. While enabled, the adjacent **LOD voxel similarity threshold** slider controls the required match percentage and defaults to `55%`; equal-face matches are retained rather than guessed. The surviving components are then renumbered continuously. Enabling **Object filtering: Resource Hash** with a non-empty Hash bypasses this automatic filter entirely, so an explicitly targeted LOD Hash retains native EFMI extraction behavior.
