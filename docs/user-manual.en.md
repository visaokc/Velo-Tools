...
**Velo Compatibility -> Slot-style textures** is optional and off by default. With the default INI template, it consumes fresh schema-v4/v5 `ShaderTextureUsage.json` evidence from the same object source and replaces covered texture Hash overrides with Component-local `ps-tN` bindings. Schema v5 is required when branch disambiguation needs complete bound-slot formats or when a CPU-posed Component needs its exact original draw range. Each assigned `ps-tN` contributes one positive condition. Complete observed-slot evidence is used only to validate negative discriminators: a legacy discriminator is retained when it is true for the target branch, replaced by the next valid candidate when possible, or omitted when the positive signature already excludes the competing branch. Export fails closed if the branches still cannot be separated. The generated draw transaction first runs EFMI's normal texture-override stage and backs up only the slots assigned by the branch that actually matched. After the Component draw, a Component-local restore command checks each backup for `null`, restores it, and clears the backup before the next draw. It does not rely on a `*_TYPELESS` matcher to stand in for typed EFMI resources. Slots are derived from the dump instead of being limited to a fixed range; current EFMI captures have been verified with `ps-t0`, `ps-t1`, and `ps-t11..22`.
