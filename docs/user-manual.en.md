...
- **Automatically skip LOD components** is opt-in and disabled by default because normal character-screen dumps usually do not contain duplicate LOD geometry. Enable it for open-world, instance, or other scene dumps that captured both lower-detail and primary meshes. Before unified-VG analysis or any JSON/file output, Velo compares the object's components and forms a pair only when both components select each other as their single highest voxel-similarity match. Each component can belong to at most one pair. Different-face-count pairs discard the lower-face member. For an equal-face pair, Velo discards the member with no PS texture bindings only when the other member has such bindings; if both or neither have PS texture evidence, both are retained rather than guessed. This uses raw PS bindings observed before texture filtering, not whether Blender eventually creates a material, so filtered or unsupported texture outputs cannot become false LOD evidence. While enabled, the adjacent **LOD voxel similarity threshold** slider controls the required match percentage and defaults to `80%`. The surviving components are then renumbered continuously. Enabling **Object filtering: Resource Hash** with a non-empty Hash bypasses this automatic filter entirely, so an explicitly targeted LOD Hash retains native EFMI extraction behavior.
...
**Velo Compatibility -> Slot-style textures** is optional and off by default. With the default INI template, it consumes fresh schema-v4 `ShaderTextureUsage.json` evidence from the same object source and replaces covered texture Hash overrides with Component-local `ps-tN` bindings. The generated draw transaction backs up every assigned slot, runs EFMI's normal texture-override stage, selects the recorded format-family branch, binds the exported resource, and restores the original slots after the Component draw. Slots are derived from the dump instead of being limited to a fixed range; current EFMI captures have been verified with `ps-t0`, `ps-t1`, and `ps-t11..22`.
