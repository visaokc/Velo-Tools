...
- **自动跳过 LOD 组件**（默认关闭）：普通角色界面 Dump 通常不包含重复 LOD，因此只在大世界、副本或其它同时捕获低模与主控网格的场景 Dump 中按需开启。在 Unified VG 分析和任何 JSON / Buffer 写入前，全量比较对象内 Component；只有双方都把对方选为自己唯一的最高体素相似度候选时才组成一对，每个 Component 最多属于一组。面数不同时删除面数更少者；面数相同时，只有一方存在 PS 贴图绑定而另一方完全没有时才删除无绑定的一方，双方都有或都没有 PS 贴图证据时仍全部保留，不作猜测。这里读取的是贴图过滤前由原始 draw 实际提供的 PS 绑定，而不是 Blender 最终是否成功生成材质，因此被过滤或不受支持的贴图产物不会被误当成 LOD 证据。勾选后显示相邻的 **LOD 体素相似度阈值** 滑块，默认 `80%`，由用户决定最低匹配率。剩余 Component 随后连续重编号。启用 **对象过滤：资源 Hash** 并填写非空 Hash 时，会完全绕过该自动过滤；因此显式指定 LOD 网格 Hash 会保持原生 EFMI 提取行为。
- **对象过滤：跳过静态对象**
- **对象过滤：最少组件数**
- **对象过滤：最少贴图数**
- **对象过滤：资源 Hash**
