...
**按贴图聚合导出**位于**按材质指定贴图**下方，默认开启。在计算 index 偏移和生成 Buffer 之前，按“原图身份 → 替换图资源”的完整对应关系，对同一 Component 的兼容临时部件稳定分组。例如 A/B/A/B 会变为 A/A/B/B，使同组 index 范围连续。源物体名称、输出注释和各部件的显隐标记保持原样；每个部件仍保留自己的 draw，不合并 GPU 绘制调用。不同图像数据块或路径引用的相同文件内容，也能识别为相同贴图组合。相邻同组 draw 共用一次贴图设置／恢复；原生单部件显隐条件内部只有 draw 时，也可共用设置，但各 draw 的开关不变。
...
导出使用重新计算后的三角面范围，保留每个 draw 的显隐条件，每组保存／恢复受影响槽位。同一原图可供多个 Component 使用不同替换图，相同文件内容复用导出资源。这是 Slot 路由，不是新增的 Hash 分 draw 匹配。已移除原图或主动停用的用途不替换；仍有连接但身份无法解析、或运行时槽位缺少安全证据时，继续中止并提示，不猜测槽位。
