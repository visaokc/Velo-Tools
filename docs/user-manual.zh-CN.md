...
传播和其它需要暂存材质副本的编辑，会在单个副本完整接替不再被使用的本地原材质时，保留当前材质的完整原名；旧备份改用 `Backup <原名>`，不再让正在使用的材质不断增加 `.001`、`.002`。原名字本来就有的数字后缀也会保留，不盲目删除。仍被其它用户引用的原材质保持名字和内容不变；确实需要隔离成两份材质时，新副本必须使用独立名称，之后继续编辑不会再累加后缀。名称转交与材质槽、同步成员一起回滚。
