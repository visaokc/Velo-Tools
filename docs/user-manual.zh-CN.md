...
生成材质与改名同步现在读取物体第一个槽位中**当前实际生效的材质**，同时支持 DATA（关联数据）和旧版 OBJECT（关联物体）覆写。仍按原功能单用户化，保留现有着色节点、贴图连线和原图映射，并把本次处理的材质使用安全整理回 DATA，不会露出下面被遮住的旧材质。同一材质在多个槽中重复使用时随副本一起更新；其它不同材质、面归属和活动槽保持不变。自动改名仍跳过多槽对象。必须复制材质时，持久贴图同步关系跟随该物体的对应材质使用位置，包含同一物体在其它 Scene 中的已有关系；保存重开及撤销／重做后保留。升级后已有名称不一致的对象，选中后执行一次**选中物体生成材质球**即可修复；不要先手动切换关联类型。
...
标准完整导出仅在网格开启 **启用 COLOR 描边法线** 且存在有效非零 `COLOR.R/G` 时，接通兼容 shader 的原生 RG 平滑法线分支。该开关位于 **网格工具 → 顶点色工具**，在 **生成平滑法线 COLOR** 按钮下方；生成成功会自动开启，已有自制 COLOR 可直接开启而无需重算。原始导入默认沿用游戏原生控制；不勾选不影响 COLOR 的绘制和导出，也不会关闭游戏已经启用的分支。同一 Component 的任一实际导出部件开启后，其兼容自定义 draw 共用激活作用域；进入时保存一次、退出时恢复，LOD 布局变化、借用其它 Component、原始 draw 和未知外部命令保留必要边界。支持 `R8G8B8A8_SNORM COLOR` 位于 12 字节 VB1 的 byte 8，或 20 字节 VB1 的 byte 16。shader 规则追踪 RG 解码到描边外扩，不假定所有 shader 或未来变体都兼容。shader 能力、原生开关状态与描边宽度是三项独立条件；从原始 FrameAnalysis CB 读取开关时，必须应用对应 draw 的 `VSSetConstantBuffers1.first_constant` 绑定偏移，不能直接读取缓冲区开头。当前开关表示作者启用意图，并非已经接入自动 dump 诊断。缺少／全零 RG、CPU-posed、不兼容布局及自定义／实时模板不接管；无 COLOR 的 LOD 保持原样。原生 RG 解码、顶点 Buffer 与材质描边宽度不修改。这不是独立粗细遮罩，也没有额外定义 `R=1` 裁描边规则。修改开关后须完整导出并重新生成 `mod.ini`；需要平滑法线时先生成再手绘，再次生成会覆盖手绘值。
...
树中的集合是真实 Blender 集合；材质叶子是预览出的虚拟分组，不代表当前已经存在对应网格对象。 已分离的真实部件在导出时以当前集合归属为准，包括普通按材质分离后手动在 Outliner 移动的情况；旧虚拟记录不能把它搬回隐藏集合。共用材质、未使用／重复材质槽和同贴图材质组均不覆盖真实归属。尚未分离的独立材质部分继续保留显式虚拟拆分目标；手动“分离并归纳到集合”操作仍按树执行。
...
1. 在物体模式选择网格，执行**初始化选中物体材质**。实际使用的 MMD／Blender 材质转换为语义节点副本，保留能识别的漫反射和 UV／向量连线；未选中用户的材质内容和槽位引用保持不变；当前编辑材质按下述规则优先取得原名。再次执行时不重建已有节点、不覆盖已有连线，只更新选中材质的原图映射。
...
4. 选中来源和目标部件，执行**按相同漫反射传播**。优先使用已配置的活动材质；活动材质没有可传播贴图时，会寻找选中范围内唯一的已配置来源。遍历选中物体的所有材质槽寻找原图身份，实际修改限于被面使用的匹配材质，不仅检查每个物体的活动材质。匹配依据为同一图像数据块或规范化后的同一保存路径，支持打包图像，以及可识别的 MMD／嵌套 BSDF 漫反射链，不按近似名称或画面相似度猜测。**连线传播与身份解析分开处理**：即使无需新增连线，也会刷新映射；选中范围内其它同漫反射材质已有的身份，会帮助解析来源和目标映射。各 Component 独立解析原图，不复制其它 Component 的槽位。新增连接默认只填空输入；**覆盖已有连接**还会替换已连接输入，并清空源材质中已不存在的非漫反射用途。源材质移除图片、图片节点或输入连线后，再次传播会把这个移除状态同步到选中且仍保持原关联的目标，即使使用默认填空模式也有效。目标已独立换图或主动解除关联时，除非开启覆盖，否则保留其独立连接。空图节点和悬空重定向会从对应语义输入断开，不删除图像数据块、不影响其它节点分支，也不清空原图身份。操作结果同时显示清空输入数量。

原始名称和提取格式用于初步分类；缺少格式记录时才参考 DDS 头。遇到多张同格式原图时，不再直接放弃：保留各个彩色 pass 的独立使用关系和记录尺寸，通过独立的漫反射／法线配对，或同 pass 中唯一的同尺寸配套关系，从辅助贴图中区分主漫反射、法线及打包 PBR／FTM。这是制作侧建议，不是固定槽位规则；证据冲突或仍有多个同等候选时才保持未分配，并显示候选数量。唯一建议直接生效，手动选择与“保留游戏贴图”保持优先。刷新可以补齐旧版未完成的映射，不重建节点。移除原先已映射的原图会停用该用途，不会自动改绑到剩余辅助图；文件放回后可恢复。原图选择器优先显示当前映射和同类候选，仍保留全部原图供手动纠正，悬停提示包含可能用途。旧工程已有映射无需补确认，未设置提取目录也可以先初始化节点。
...
不会跨 Component 或越过保序部件聚合。CPU-posed Component、已有终末地证据明确标记为透明的 Component、未进入贴图指定流程的材质，以及尚未拆开的混合贴图网格保留位置。Blender 预览材质的 Alpha 连线、Alpha 数值和混合方式不等于导出的游戏混合状态，不再因此自动取消已开启的聚合。对于已有证据尚未识别的透明或其它依赖绘制顺序的游戏 pass，请在材质工具勾选**保留绘制顺序**。这些部件作为排序边界，两侧相同贴图不会越过它合组。关闭**按贴图聚合导出**可恢复此前的物体顺序和仅相邻合组行为，不影响贴图指定及同步。

导出使用重新计算后的三角面范围，保留每个 draw 的显隐条件，每组保存／恢复受影响槽位。同一原图可供多个 Component 使用不同替换图。替换图会按照节点所记录路径中的**原始文件名**复制到 `Textures/`，不增加前缀、摘要后缀，不改变大小写或扩展名。目标目录已有同名贴图时直接保留并跳过，不比较内容、尺寸或 Hash；只新增缺少的文件。输入端不同来源的贴图争用同一个 Windows 等价文件名时，仍会中止并提示，不静默选择其中一张。需要更新已存在的输出贴图时，由作者自行处理。旧版本已经产生的 `material_<摘要>.*` 文件不会自动删除。这是 Slot 路由，不是新增的 Hash 分 draw 匹配。已移除原图或主动停用的用途不替换；仍有连接但身份无法解析、或运行时槽位缺少安全证据时，继续中止并提示，不猜测槽位。

材质替换图的 Resource 节名使用 `ResourceMaterialTexture_<文件主名>`：去掉末尾扩展名，空格换成 `_`，保留英文字母、数字和下划线，其它字符忽略；不附加内容摘要，也不为无冲突名称附加编号。合法文件在清理后节名相同时，自动追加 `_001`、`_002` 等区分，**不改磁盘文件名和 `filename` 引用**。例如 `body_1.dds` 使用 `ResourceMaterialTexture_body_1`，`body 1.dds` 使用 `ResourceMaterialTexture_body_1_001`。相同文件集合的编号确定且不依赖 draw 顺序；先保留文件自身已有的编号名称，如 `body_1_001.dds`，再分配不占用它的后缀。不同扩展名但主名相同也按此处理。主名清理后没有可用字符时需修改源图名称。**节名碰撞不等于磁盘文件冲突**：输出目录已有同名不同内容文件仍遵守前述不覆盖规则。贴图聚合按实际文件身份判断，不会因为清理后的名字相同就误合并不同贴图。

只有包含插件贴图指定节点的材质进入这层额外替换；未初始化的 MMD／BSDF 材质沿用原导出路径，其 draw 也作为排序边界。用户显式执行同漫反射传播时，可以初始化选中范围内的匹配目标，这些目标之后才加入新路线；导出本身不会初始化未处理材质。

贴图同步分组与材质槽的关联类型无关。初始化、传播、映射编辑和换图时，本地单用户网格原有的 DATA 关联保持不变；仅在共享网格或外部链接网格需要隔离其它用户时采用 OBJECT 覆写，避免只为换图复制几何。已有 OBJECT 材质同样能正常使用；生成同名材质完成单用户化后可安全整理回 DATA。

传播、刷新原始匹配和其它需要暂存副本的编辑，与生成同名材质采用一致的原名优先原则：一对一替换的当前材质优先取得本地原材质的完整名字。即使旧材质仍被其它用户或未使用的网格引用，也只将旧 ID 改为 `Backup <原名>`，不删除其内容、不改动其它用户的槽位引用；当前编辑材质不再因此增加 `.001`、`.002`。操作前已有的合法后缀不会被盲目删除或猜测。外部链接材质以及同一原材质同时生成多份不同结果的情况，仍受 Blender 名称唯一性限制。名称转交与材质槽、同步成员一起回滚。

材质 INI 状态只作用于实际替换贴图的 Component 和槽位。能确定最终分支布局的选择器，每个受影响 Component 共用一个 `$material_layout_cN` 标记；复杂的独立条件分支只跟踪真正需要的槽位。不再向无关 Component 的设置／恢复节插入变量，共享辅助命令需要跟踪时只为受影响的调用方隔离处理。备份资源按 Component／槽位共用，不随材质变体重复声明。沿用原选择器在替换前作出的判断，不根据替换图格式重新猜槽位，也不增加运行时 Hash 判断。

贴图设置放在物体注释之前；每条生成的 draw 正上方保留原物体注释，材质分段与原生显隐条件同样处理。预检、聚合和 INI 生成共用一次导出内的提取快照、映射结果、图像字节与内容签名缓存，成功或失败后都释放；普通未处理材质跳过材质分段，同一绑定的网格直接生成一个范围。仍校验真实文件名冲突，不覆盖已有贴图。更新插件并完整重导出后，旧的冗长 INI 才会被替换；不需要重新初始化材质或重新传播。
...
对于 **Merged（合并骨架）**输出，Velo 会在写入校验和前检查渲染后的 `[ResourceMergedSkeletonDataRW]`。EFMI 模板若没有完整声明资源用途，会自动补入 `bind_flags = shader_resource unordered_access`，避免 XXMI 因目录名改变解析顺序而创建只有 UAV、没有 SRV 的骨架 Buffer。该逻辑是与官方模板分离的兼容层：一旦上游声明已经显式包含这两项标志，Velo 会自动识别并保持其输出逐字不变。仅将资源类型改为 `RWStructuredBuffer` 不会停用兼容层，因为 XXMI 对 RW／非 RW 类型名称的绑定能力处理相同，资源类型本身不能证明具备 SRV 与 UAV。其它骨架模式，以及没有定义这个精确目标资源节的自定义模板，不会被修改。检测针对每次实际渲染的导出内容，不依赖缓存版本号或远端发布公告；导出过程不联网。即使使用过新模板，之后导出旧模板仍会保留所需补丁。后续集成官方修复并验证所有受支持路径在关闭补丁后仍正常，才能移除独立兼容模块及其注册挂载，不需要修改官方模板或已保存工程。
