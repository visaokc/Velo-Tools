...
> 适用版本：Velo Tools v1.4.1。本文以当前中文 UI 为准；`IB`、`VB`、`Hash`、`Merged`、`Per-Component`、`Frame Dump`、`LOD`、`INI`、`ShapeKey`、`DDS` 等技术标识保留原文。
...
- 左侧复选框用于选择参与**自动重命名**的未编号项，默认全部不选；表头复选框可全选或全不选全部未编号项。
...
- 鼠标悬浮在 `xN` 上可查看包含该 ShapeKey 的全部网格对象。名称列优先获得侧边栏新增宽度，value 滑块最多占 10 个 Blender UI units。
