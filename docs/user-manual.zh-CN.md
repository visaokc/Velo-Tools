...
**材质工具**位于**权重工具**右侧。这里把两件事分开：把新图连接到“漫反射、法线”等用途输入，再选择该用途对应的原始提取贴图。用户不填写 `ps-t` 编号；导出器通过原始资源身份和已有 Component/pass 感知的 Slot 规划器定位实际槽位。

在对应游戏的**导出 Mod**兼容选项中，开启**导出时自动按材质拆分**、**插槽风格贴图**和**按材质指定贴图**，同时开启 INI 输出及复制贴图，使用独立的 Mod 输出目录。新选项默认关闭，关闭自动材质拆分时隐藏；关闭材质拆分也会完全跳过新贴图绑定路径。

1. 在物体模式选择网格，打开**材质工具**，选择正确游戏并执行**初始化选中物体材质**。实际使用的 MMD 或 Blender 材质转换为带语义输入的节点组副本；能识别的漫反射图及其 UV/向量连线会保留，原材质作为备份保留。采用物体级材质指定，不影响未选中但共享网格的物体；重复初始化不会覆盖已有指定节点。
2. 在着色器编辑器中，将图像纹理节点的**颜色（Color）**输出连到 **Diffuse（漫反射）**、**Normal（法线）**、**Packed PBR（终末地打包 PBR）**、**FTM（鸣潮）**、**Mask（遮罩）**、**Emission（自发光）**、**Light Map（光照贴图）**或 **Detail（细节贴图）**。保持 **Shader** 输出连接到活动材质输出。支持直接图像节点和重定向；程序化贴图先烘焙。未连接的用途保留原有 Slot 行为；Alpha 只控制预览，不是独立导出贴图用途。
3. 设置原始对象源目录，执行**刷新原始贴图映射**。优先用材质名的 Component 前缀，否则用物体名前缀确定所属部件。插件依据原始文件名、提取格式，以及与原始漫反射同一 pass 出现的关系缩小候选；提取记录缺少格式时才读取原始 DDS 头作为提示。**格式只能提示，不能证明用途。**核对建议的原图，必要时通过**选择对应的原始贴图**按缩略图/文件名选择，再执行**确认建议的贴图对应关系**。已直接匹配原始身份的漫反射不需要重复确认。尚未配置提取数据也能初始化节点，但有连接的贴图必须完成映射后才能导出。
4. 传播其它贴图时，把配置好的材质设为活动材质，选中目标物体，执行**按相同漫反射传播**。只匹配同一个图像数据块或同一个已保存图像的规范路径，不按相似文件名或画面内容猜测。默认只填空的非漫反射输入，可勾选**覆盖已有连接**。各目标仍使用自己的 Component 原图映射，特别是跨 Component 传播后要分别核对。

导出器记录最终三角化后的材质面范围，保留 draw 顺序和条件，只合并同一 Component 内相邻且兼容的贴图设置组。每组在绘制前保存并替换相关槽位，绘制后恢复；不同 Component 即使共享一张原始贴图，也能为各自的 draw 指定不同替换图。相同替换文件内容复用导出资源。这是 Slot 路由，不是新增的运行时 Hash 分 draw 匹配。

本轮支持单来源 EFMI、WWMI 的 Native/Fuzzy Slot 导出。CrossIB、鸣潮 Cross-Scene、资产名导出、自定义/实时模板和部分导出暂时明确拒绝。使用已连接材质贴图的部件必须参与安全 Slot 导出；没有可用原图槽位绑定时中止，不静默回退。PBR/FTM 打包图原样复制，不解包通道；Principled 预览为近似效果，不等价于游戏着色器。支持已保存的 DDS/PNG/JPG/JPEG/TGA/BMP 及内嵌文件图像，不自动转换格式；生成图、未保存修改、动画、分块或程序化图像应先保存/烘焙为受支持的单张图像。初始化可用 Blender 撤销，或重新指定保留的原材质回退；导出后仍需游戏内验证。
