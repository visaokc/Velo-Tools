...
标准完整导出会对兼容的描边 shader 自动接通原生 RG 平滑法线分支：只要一个 Component 的任一导出部件含非零 `COLOR.R/G`，该 Component 的全部自定义 draw 就共用激活作用域，包括 RG 全零的部件。生成、手绘及保留的原始导入值都参与判断，无需新增开关或制作标记。状态在进入时保存一次、退出时恢复一次，不再为每个材质／显隐子 draw 重复保存与恢复；LOD 布局变化、借用其它 Component、原始 draw 和不透明的外部命令仍保留必要的状态边界。支持 `TEXCOORD.xy` 后的 `R8G8B8A8_SNORM COLOR`：12 字节 VB1 中 COLOR 位于 byte 8，或 20 字节 VB1 中在第二 UV／保留的 8 字节字段后位于 byte 16。shader 规则覆盖已验证的材质布局变体，并追踪 RG 解码到描边外扩的数据流，不假定所有 shader 或未来变体都兼容。整个 Component 缺少／全零 RG、CPU-posed、不兼容布局及自定义／实时模板不接管；无 COLOR 的 LOD 保持原样，不妨碍兼容主模型启用。原生已开启结果、RG 解码、顶点 Buffer 和材质描边宽度不修改。这不是独立粗细遮罩，也没有额外定义 `R=1` 裁描边规则，手涂后的实际效果以游戏测试为准。必须完整导出并重新生成 `mod.ini`，不能只导出 Buffer。需要平滑法线时先生成，再手绘；再次运行生成器会覆盖手绘值。
