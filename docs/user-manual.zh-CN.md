...
镜像严格来自本侧结果，不独立读取游戏另一侧。重合顶点共用镜像 bucket 值；只复制互相对应的镜像 bucket，无法互相对应时明确报告。传递成功后仍可自动锁定本次承接组，不妨碍继续传递其它组。
...
两种引擎共用无规格化的承接写入计划和完整写后校验；失败时恢复原有 membership、权重及锁标记。最终的按比例规格化仍遵守锁定状态：先手动解锁要参与计算的组，再执行组数裁剪和规格化，工具不会擅自解锁。规格化会调整保留权重的比例，不能修正采样位置错误或消除平滑造成的形状变化。旧算法已写坏的组需要重新传递，更新插件不会自动重建丢失的权重。
