...
标准完整导出现在会对兼容的描边 shader 自动接通原生 RG 平滑法线分支，条件是导出网格含非零 `COLOR.R/G`。生成、手绘及保留的原始导入值都参与判断，无需新增开关或制作标记。当前支持 12 字节 VB1 中的 `TEXCOORD.xy` 加 `R8G8B8A8_SNORM COLOR` 布局；缺少／全零 RG、CPU-posed Component、不兼容布局及自定义／实时模板不接管。布局资格随实际绑定的 VB1 判断，因此缺少 COLOR 的 LOD 保持原样，但不会阻止兼容的主模型启用。仅在对应自定义 draw 期间扩展原生开关，绘制后立即恢复状态；原生已启用的材质、RG 解码、顶点 Buffer 和材质描边宽度均不修改。这不是独立的粗细遮罩，也没有额外定义 `R=1` 裁描边规则，手涂后的实际效果以游戏测试为准。必须完整导出并重新生成 `mod.ini`，不能只导出 Buffer。需要平滑法线时先生成，再手绘；再次运行生成器会覆盖手绘值。
