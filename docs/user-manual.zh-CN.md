...
- **自动供体数**控制优先供体建议数量，不是允许参与规格化的组数上限。
- 自动列出的供体是预览建议。
- 手动填写的供体也是优先建议；其它未锁定的普通组仍可按比例让出或接收余量。
- 无效建议不阻止可行传递；局部缺少余量权重时尝试最近空间证据，不要求先把权重集中到一个供体组。
- 锁定的普通顶点组会作为受保护容量处理。

镜像严格来自本侧结果，不独立读取游戏另一侧。两侧预算联动：承接权重优先，遇到锁定容量限制时共同调整；最后一个可用组数槽位可吸收余量。自动调整数量写入报告。旧承接权重变成零的区域同样参与规格化。传递成功后可自动锁定承接组，后续逐组传递不会改动已锁定组。
...
两种引擎共用写前预算检查和写后校验；锁定预算无解或找不到余量证据时取消操作并恢复原有 membership 和权重。即使报告成功，也应保存工程并检查 Deform 结果，而不是只看操作器状态。
