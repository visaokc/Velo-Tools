...
- **自动跳过 LOD 组件**（默认开启）：在 Unified VG 分析和任何 JSON / Buffer 写入前，按原始 draw 顺序只让疑似低模 draw 与后方一个小型局部窗口比较，并选择其中体素相似度最高、面数更多的候选。勾选后显示相邻的 **LOD 体素相似度阈值** 滑块，默认 `55%`，由用户决定最低匹配率；面数相同则不猜测删除。剩余 Component 随后连续重编号。启用 **对象过滤：资源 Hash** 并填写非空 Hash 时，会完全绕过该自动过滤；因此显式指定 LOD 网格 Hash 会保持原生 EFMI 提取行为。
- **对象过滤：跳过静态对象**
- **对象过滤：最少组件数**
- **对象过滤：最少贴图数**
- **对象过滤：资源 Hash**
