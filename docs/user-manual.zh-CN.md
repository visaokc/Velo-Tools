...
**匹配用姿势**位于骨架选择框下方，通用映射与 MMD 映射分别独立设置。默认 **静态位置**使用未变形网格的顶点组加权中心，并非骨骼头位置；**姿态位置**使用两边网格当前求值后的加权中心，包含可见的 Armature、约束、驱动器、ShapeKey 和修改器变形。连线、未匹配点与鼠标拾取随姿态变化同步移动，但映射关系仅在再次点击 **按位置匹配** 时重算。旧静态匹配表无需清空即可重新匹配。该选项不会应用修改器、切换骨架显示状态或修改网格；骨架选择框仍用于同步改名，变形取自各网格实际的修改器链。

映射表可以保存在 `.blend` 内的文本数据中，也可以导入或导出为外部文本。

**MMD 源**只是编辑、匹配的参照，不是导出白名单。导出时，当前 MMD 映射表按顶点组名称应用于所选 **组件集合** 内所有符合条件的网格，包括分离后的网格和未单独绑定映射表的网格；子集合及隐藏对象/集合仍遵守导出选项。改名映射、特殊组清理（`mmd_edge_scale`、`mmd_vertex_order`、`UV_*`）和空组清理都在临时副本上进行，不改原网格或工具中的选择。不同骨架模式仍保留各自的后续索引转换和校验规则。
...
MMD 和权重工具的网格选择框保存用户指定的**精确名称**，不永久绑定某一个对象指针，也不限制网格正常改名或合并。工具始终查找与保存名称完全一致的网格；无可用匹配时框变红、名字不变，同名网格再次出现（包括重新合并后）就自动接上并恢复正常颜色。等待期间也可以按 **X** 清空绑定，再指定其它名字；清空后不再等待旧名字。不采用近似名称、数字后缀或其它名称的合并结果作为替代。

按材质分离会在**进入编辑模式之前先选 Basis**，分离后仍有 ShapeKey 的所有碎片也保持选中 Basis；按材质分离所属集合、按贴图分离的共用拆分阶段同样处理。这样可防止清除空键后，原生合并使用此前活动键的网格坐标填充缺失键。仅在分离后切 Basis 不等价；这项预防不会自动修复已经污染的坐标。

分离控件附近的 **清理污染形态键** 按钮在物体模式下一键处理所选网格，**不需要干净副本**。检测要求整个材质区域的位移在至少三个整体形状不同的键中完全重复，并能找到自成一体的来源键。先保护来源本体，再把其它键中纯重复的区域还原到 Basis；同一区域有不同或叠加正常变形时不动。来源允许小幅后期微调，但近似匹配只用于保护来源，不用于近似扣除。完全相同的来源副本全部保留；多个不同本体候选、来源不明的重复模式跳过。识别不依赖键名、顺序、活动键或滑条值。

这是保守识别，不是单凭重复就能证明污染：若作者故意让多个组合键完整复用同一来源位移，坐标本身无法区分其与污染。支持 **Ctrl+Z 撤销**，执行提示会列出保护的来源、清理记录数和跳过数量。不删除 ShapeKey、不改滑条或相对关系，写入失败时回滚整个批次。共享或只读数据、绝对键、非 Basis 相对键、带顶点组遮罩的键，以及超出 512 MiB 坐标快照预算的网格会跳过；跨材质共用顶点的区域合并判断，不细拆后强行投票。该按钮不使用旁边的零位移清理阈值，也不尝试扣除任意叠加污染。
...
如果终末地目标 Component 的提取布局使用 `TEXCOORD4.xy` 保存切线空间平滑法线 X/Y，选中网格后，在 **UV工具** 中执行 **生成平滑法线 TEXCOORD4**。工具在计算平滑方向时会焊接坐标完全相同的重复顶点，再以 `TEXCOORD.xy` 建立切线基，并把 EFMI 导入空间所需的表示写入 `TEXCOORD4.xy`。

该生成器用于新增网格，不是删除原资产烘焙数据后的 bit-exact 恢复工具。原始顶点应保留提取层；条件允许时，先在新增网格对象上生成辅助层，再把它合并进 Component。

如果目标 Component 布局把同一种表示映射到压缩 `COLOR`，则在新增的 **顶点色工具** 中执行 **生成平滑法线 COLOR**。不要把它用于任意带 COLOR 的 Component：终末地 COLOR 是承载数据的容器，不是统一 semantic，部分原生 Component 的 COLOR 保存了不同的美术数据。执行前应检查源 `.fmt` 和保留的参考数据。

无需合并 UV 岛。UV 接缝两侧坐标相同的重复顶点会共同参与平滑；但是修改主 `TEXCOORD.xy` 的方向或形状会改变切线基，因此也会改变生成值。已有 `TEXCOORD4.xy` 或 `COLOR` 会被覆盖。
...
6. 按需设置平滑；传递阶段不规格化、不裁剪组数，也不需要供体。
7. 执行 **权重传递**，可保持传递后的承接组上锁，继续处理其它组。
8. 全部传递完成后，在 Edit Mode 选择要整理的顶点，手动解锁需要参与整理的组，再执行 **按比例规格化选中顶点**。
9. 按需恢复锁定，在 Weight Paint 和姿态模式中检查最终结果。
...
两种引擎均只写入本次承接组及其严格镜像。平滑只处理本次采样结果，在最终镜像之前执行，不会顺带规格化其它组。

其它组无论锁定与否，其权重和 membership 都保持不变；它们的总和和组数不再限制本次承接值。不要求预先集中供体，不借用其它组的余量，也不为凑齐总和或组数而放大、缩小本次权重。若本次承接组自己已锁定，仍需用户先明确解锁才可重做。

镜像严格来自本侧结果，不独立读取游戏另一侧。相近的多层网格分别匹配，不再按坐标网格合桶平均，本侧采样值保持不变。完全重合的顶点优先利用可区分的拓扑；仍有冲突时不平均、不猜层。无法互相匹配或存在歧义时保留已有目标权重并报告。传递成功后仍可自动锁定本次承接组，不妨碍继续传递其它组。

工作阶段允许总和暂时小于或大于 1，也允许暂时超过最终组数上限。即使旧工程保存了“执行后规格化”，也不会重新启用传递时的自动规格化；统一整理由用户在传递结束后手动执行。

#### Robust 证据边界

Robust 会过滤没有正权重直接来源种子的正值连通域，包括连通网格内部由 inpaint 外推形成的孤立正值斑块。该过滤不等于解剖语义匹配保证。

该保护减少明显污染，但不等于自动得到正确骨骼语义。传递后仍应检查接缝、裙摆、头发、左右侧和与身体分离的小岛。

#### 后处理与局部修复

- **启用平滑**默认关闭；开启后可设置次数和强度，并默认避开 UV 接缝。已有工程明确保存的设置仍保留。
- **限制每顶点组数量**只用于手动整理和单独镜像，不限制源权重传递。
- **单独镜像后规格化**及供体数量位于镜像面板，保留原有单独镜像行为。
- 选中顶点修复可在 Edit Mode 中处理局部问题。
- 未解决顶点会保持选中，便于继续人工检查。

两种引擎共用无规格化的承接写入计划和完整写后校验；失败时恢复原有 membership、权重及锁标记。最终的按比例规格化仍遵守锁定状态：先手动解锁要参与计算的组，再执行组数裁剪和规格化，工具不会擅自解锁。规格化仅写所选顶点的变化，不清理未选顶点的微小权重或零 membership。规格化会调整保留权重的比例，不能修正采样位置错误或消除平滑造成的形状变化。旧算法已写坏的组可重新传递，或使用下方镜像修复；更新插件本身不会自动改写用户工程。

#### 镜像选中顶点权重

在按比例规格化按钮下方选择 **-X 到 +X** 或 **+X 到 -X**，再执行 **镜像选中顶点权重**。方向使用网格局部 X 轴，不是屏幕左右。可以只选出错的孤立顶点，从对面读取正确权重，**不要求同时选中对面的来源顶点**；也可以选健康一侧，将其权重复制给对应目标。执行后的左下角 **调整上一步操作**（F9）可再次选择方向，Ctrl+Z 撤销。
...
在对应游戏的**导出 Mod**兼容选项中，开启**导出时自动按材质拆分**、**插槽风格贴图**及**按材质指定贴图**，同时开启 INI 输出和复制贴图，使用独立 Mod 输出目录。新功能默认关闭；关闭材质拆分后隐藏选项，并完全跳过材质贴图绑定。

1. 在物体模式选择网格，执行**初始化选中物体材质**。实际使用的 MMD／Blender 材质转换为语义节点副本，保留能识别的漫反射和 UV／向量连线；未选中的共享网格／材质用户不受影响。再次执行时不重建已有节点、不覆盖已有连线，只更新选中材质的原图映射。
2. 在着色器编辑器中，将图像纹理的 **Color** 输出连接到 **Diffuse、Normal、Packed PBR、FTM、Mask、Emission、Light Map、Detail** 等输入，保持 **Shader** 连接到活动材质输出。导出支持直接图像节点和重定向；程序化图先烘焙。Alpha 控制预览，不是单独导出贴图用途。
3. 设置正确的对象源目录，使用**刷新原始贴图映射**更新活动材质。**只有源目录当前保留的图像文件参与候选。**不想处理跨很多 Component 的公共贴图，可以将其移出源目录，或移入备份子目录；即使 `ShaderTextureUsage.json` 仍记录它，也不会再作为候选。初始化／刷新会移除其有效映射，保留 Blender 中已有图像连线，原先映射到已移除原图的用途改为**保留游戏贴图**。把原图放回后可重新恢复映射。无需修改提取 JSON，也不操作游戏资源。手动改过的原图选择会保留；主动选择**保留游戏贴图**可停用某个用途的替换。
4. 选中来源和目标部件，执行**按相同漫反射传播**。优先使用已配置的活动材质；活动材质没有可传播贴图时，会寻找选中范围内唯一的已配置来源。遍历选中物体的所有材质槽寻找原图身份，实际修改限于被面使用的匹配材质，不仅检查每个物体的活动材质。匹配依据为同一图像数据块或规范化后的同一保存路径，支持打包图像，以及可识别的 MMD／嵌套 BSDF 漫反射链，不按近似名称或画面相似度猜测。**连线传播与身份解析分开处理**：即使无需新增连线，也会刷新映射；选中范围内其它同漫反射材质已有的身份，会帮助解析来源和目标映射。各 Component 独立解析原图，不复制其它 Component 的槽位。默认只填空输入；需要替换已连好的图时勾选**覆盖已有连接**。操作结果会显示来源、匹配材质数、新连线数、刷新映射数及待指定用途数。

自动候选结合原始名称、提取格式和漫反射同 pass 共现关系；缺少格式记录时才参考 DDS 头。唯一结果直接使用，真正无法唯一判断的用途保持未指定，用户直接选图即可。旧工程已有映射无需补确认，未设置提取目录也可以先初始化节点。

**按贴图聚合导出**位于**按材质指定贴图**下方，默认开启。在计算 index 偏移和生成 Buffer 之前，按“原图身份 → 替换图资源”的完整对应关系，对同一 Component 的兼容临时部件稳定分组。例如 A/B/A/B 会变为 A/A/B/B，使同组 index 范围连续。源物体名称、输出注释和各部件的显隐标记保持原样；每个部件仍保留自己的 draw，不合并 GPU 绘制调用。替换图内容与原始文件名均相同时，即使由不同 Image 数据块或不同目录载入，也使用同一贴图组合；文件名不同则保持独立，避免导出时改名。相邻同组 draw 共用一次贴图设置／恢复；原生单部件显隐条件内部只有 draw 时，也可共用设置，但各 draw 的开关不变。

不会跨 Component 或越过保序部件聚合。已知混合材质、Alpha 有连线或小于 1 的预览材质、CPU-posed Component、已有终末地证据明确标记为透明的 Component，以及尚未拆开的混合贴图网格保留位置。其它依赖绘制顺序的游戏 pass，请在材质工具勾选**保留绘制顺序**：Blender 预览设置不能证明所有游戏 pass 都可任意重排。这些部件作为排序边界，两侧相同贴图不会越过它合组。关闭**按贴图聚合导出**可恢复此前的物体顺序和仅相邻合组行为，不影响贴图指定及同步。

导出使用重新计算后的三角面范围，保留每个 draw 的显隐条件，每组保存／恢复受影响槽位。同一原图可供多个 Component 使用不同替换图。替换图会按照节点所记录路径中的**原始文件名**复制到 `Textures/`，不增加前缀、摘要后缀，不改变大小写或扩展名。目标目录已有同名同内容文件时直接复用且不重写；已有同名不同内容文件，或两张输入图在 Windows 下发生仅大小写不同的同名冲突时，导出会中止，绝不覆盖任一文件。此时应修改源图文件名，或改用干净的输出目录。旧版本已经产生的 `material_<摘要>.*` 文件不会自动删除。这是 Slot 路由，不是新增的 Hash 分 draw 匹配。已移除原图或主动停用的用途不替换；仍有连接但身份无法解析、或运行时槽位缺少安全证据时，继续中止并提示，不猜测槽位。

支持单来源 EFMI／WWMI 的 Native／Fuzzy 导出；CrossIB、鸣潮 Cross-Scene、资产名导出、自定义／实时模板及部分导出仍不支持。PBR／FTM 原样复制，Principled 预览不等价于游戏着色器。支持已保存 DDS／PNG／JPG／JPEG／TGA／BMP 和内嵌文件图像，不自动转换格式；未保存修改、生成图、动画、分块或程序化图像先保存／烘焙。可使用撤销或重新指定保留的原材质回退，最终显示仍需游戏内验收。

#### 持久同步组与实际贴图路径

传播现在会记住明确的**物体／材质／贴图用途**成员。首次仍由用户选中来源和目标网格，再执行**按相同漫反射传播**；同一网格内多个材质会分别检查，只纳入匹配且实际被面使用的材质。来源只有漫反射也能建立组：已经识别原图、尚未连接替换图的用途可以关联，后续再统一指定法线或 PBR。默认填空模式不会把目标中独立配置的不同贴图强行加入组。旧版传播没有保存成员关系，升级后需要重新传播一次建立同步组；初始化不会全场景猜测分组。

每个原图选择框下方显示**节点实际使用的图像路径**。路径框只读，旁边的文件夹按钮打开**更换贴图**，可浏览或输入新路径；**同组：N 个材质**按钮可查看成员。传播后，只选任意成员，就能统一更换该用途的全部已关联成员，不必重新选中其它物体；本项独立时只更换当前物体的当前材质使用位置。内嵌图会明确标记，框中是其记录的原路径，不代表当前必须从该磁盘路径读取。

换图只改目标材质的图像节点，不全局改写共享 Image 的文件路径。各 Component 的原图身份、实际槽位、UV／向量连线和未参加传播的用户不变。更换漫反射不改变同步组身份；物体／材质／图像改名、保存重开工程和撤销／重做后关系仍保留。复制物体不会自动加入原组。在节点编辑器中单独改某项图像，会作为本项局部覆盖，不再跟随之后的同组换图；需要恢复同步时显式重新传播。不按当前路径定期扫描全场景扩大范围。

| 操作 | 原图对应 | 图像连线 | 同步关系 |
| --- | --- | --- | --- |
| **退出本项同步** | 保留 | 保留 | 仅当前物体的当前材质、本用途退出，其它成员及用途不变 |
| **清空原图对应**／**未分配** | 清空，同时取消手动停用；刷新或导出准备可再次自动解析 | 保留 | 保留 |
| **保留游戏贴图** | 明确停用本项材质替换，刷新不自动启用 | 保留，供编辑／预览 | 保留；之后同组换图也不会擅自重新启用本项导出 |

**未分配是尚未解析的状态，不是永久关闭开关。**启用且已连图的用途必须在导出前解析到安全原图；已知映射错误会在创建临时导出网格前检查。**保留游戏贴图**跳过这层材质替换，沿用该 Component 原有 Slot 处理，并不删除源图或撤销其它资源覆盖。上述三个操作都不会断开整组材质节点。文件加载或组提交失败时，新建图像、材质指定及成员关系整体回滚；取消选择器不改工程。
...
- **自动跳过 LOD 组件**（默认关闭）：普通角色界面 Dump 通常不包含未启用的 LOD draw，因此只在大世界、副本或其它场景 Dump 中按需开启。在 Unified VG 分析和任何 JSON / Buffer 写入前，Velo 直接移除原始 draw 数据中没有任何 PS 贴图绑定的 Component，再连续重排保留项。这里读取的是贴图过滤前的原始 PS 绑定，不以 Blender 最终是否生成材质为准。启用 **对象过滤：资源 Hash** 并填写非空 Hash 时，会完全绕过该自动过滤；因此显式指定 LOD 网格 Hash 会保持原生 EFMI 提取行为。
- **组件过滤（保留）**填写要保留并送入后续分析与写入的 Component 序号或区间，例如 `0-8` 或 `0,1,5-7`。
- **组件过滤（跳过）**使用相同语法填写要排除的 Component。两个输入框都按自动 LOD 过滤后的序号计算，“跳过”优先于“保留”，最终结果再次连续重编号。例如“保留”填 `0-8`、“跳过”填 `4,6`，输出的是 LOD 过滤后原序号 `0-3,5,7-8`。
- **对象过滤：跳过静态对象**
- **对象过滤：最少组件数**
- **对象过滤：最少贴图数**
- **对象过滤：资源 Hash**
...
MMD 预处理覆盖所选导出集合，而非仅“MMD 源”指定的一个网格。**Merged（合并骨架）**按共享 runtime 骨架解析，保留已有的跨 Component 骨骼查找；**Merged（统一顶点组）**回译到 Component-local 索引；**Per-Component**继续原生局部路径。不能仅因为当前 Component 没有某个局部条目，就拒绝合并骨架模式中有效的权重。
...
**Velo 兼容选项 -> 导出时自动按材质拆分**默认开启，统一控制完整的材质识别导出链路：

- **开启：**保留智能材质名识别、临时材质拆分、材质到集合的路由，以及适用的材质归属和修改器检查。材质中的 `Component N` 前缀可以参与目标 Component 的选择，既有路由规则保持不变；不会破坏性拆分场景对象。
- **关闭：**完整跳过导出中的材质名路由、校验和拆分。每个物体整体按自身的 `Component N` 名称归属，而不是按内部材质归属；任意材质名、冲突的材质前缀和多个材质槽都不会改变 Component 划分。物体名不合格时，不会因为材质有前缀而被提升为可导出对象。导出期间及收尾的材质树刷新也会跳过。

这个开关不会关闭 MMD 映射、骨架模式规则、修改器应用、ShapeKey 处理或对象/集合隐藏筛选。独立的手动按材质拆分和网格命名工具仍保持各自行为。
...
**Velo 兼容选项 -> 导出时自动按材质拆分**在 WWMI 单 IB、Cross-Scene 和 `Per-Component (from Merged)` 中遵循相同的开关语义。开启时保留现有智能材质路由与检查；关闭时全程按物体名划分 Component，包括 Cross-Scene 的 body、own-buffer 和 editable 单元预处理，不读取材质名，也不因材质前缀而推迟权重转换。既有全局到局部的 Component 映射、顶点组合法性检查和对象/集合隐藏筛选仍然生效。
