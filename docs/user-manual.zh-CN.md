...
导出保留最终三角面范围、draw 顺序及显隐条件，只对同一 Component 内相邻且兼容的绑定合组，每组保存／恢复受影响槽位。同一原图可供多个 Component 使用不同替换图，相同文件内容复用导出资源。这是 Slot 路由，不是新增的 Hash 分 draw 匹配。已移除原图或主动停用的用途不替换；仍有连接但身份无法解析、或运行时槽位缺少安全证据时，继续中止并提示，不猜测槽位。
