...
class VELO_PT_mesh_actions(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        info = layout.row()
        info.enabled = False
        info.label(text=iface_('Currently selected mesh: {0}').format(sel_count), icon='RESTRICT_SELECT_OFF')
        ...
        prefix_row.operator(
            ...
            text='Add the Component prefix to the selected object',
        )
        ...
        material_row.operator("velo.gen_material_for_selected",
                              icon='MATERIAL', text='Generate a material ball for selected objects')
        material_row.prop(
        ...
        sub.operator("velo.split_by_material",
                     icon='MOD_EXPLODE', text='Separate by material')
        ...
        sub2.operator("velo.merge_by_texture",
                      icon='AUTOMERGE_ON', text='Merge according to textures')

        # —— Utilities (from the VTEF toolbox) ——
        layout.separator()
        util = layout.column(align=True)
        util.scale_y = 1.05
        util.label(text='Utilities', icon='TOOL_SETTINGS')
        ...
        if sel_count == 0:
            box = layout.box()
            box.scale_y = 0.8
            box.label(text='Please first select the mesh object in the view', icon='INFO')
...
class VELO_PT_shapekey_panel(bpy.types.Panel):
    bl_label = 'ShapeKey Summary (by Collection)'
    ...
    def draw(self, context):
        ...
        row.operator("velo.refresh_shapekey_list",
                     icon='FILE_REFRESH', text='Force refresh')
        ...
        row.operator(
            ...
            text='Select None' if all_selected else 'Select All',
            icon='CHECKBOX_HLT' if all_selected else 'CHECKBOX_DEHLT',
            depress=all_selected,
        )
        row.operator("velo.rename_shapekeys_deform",
                     icon='SORTALPHA', text='Automatically rename')

        n = len(s.shapekey_items)
        if n == 0:
            box = layout.box()
            box.scale_y = 0.8
            box.label(text='After selecting the collection, it will automatically scan (only scans mesh vertices ShapeKey)', icon='INFO')
            return

        layout.label(text=iface_('A total of {0} ShapeKey (Auto-sync; renaming will refresh all grids)').format(n))
        ...
        box = layout.box()
        box.scale_y = 0.8
        box.label(text='Automatically rename unchecked items; hover over x N to view the parent object', icon='INFO')


class VELO_PT_mesh_material_routing(bpy.types.Panel):
    bl_label = 'Split into Collections by Material'
    ...
    def draw(self, context):
        ...
        if root is None:
            ...
            box.label(text=iface_('Please first specify the component set in {0} Export Mod').format(display), icon='INFO')
            return

        layout.label(text=iface_('Export root collection: {0}').format(root.name), icon='OUTLINER_COLLECTION')
        if getattr(_ops.get_export_component_settings(context.scene), "ignore_nested_collections", False):
            warn = layout.box()
            warn.alert = True
            warn.label(text='Currently still ignoring nested collections; this option will automatically be disabled when performing operations in this panel', icon='ERROR')
        ...
        tree_tools.operator("velo.route_add_child_collection", icon='ADD', text='Add a new subset')
        tree_tools.operator("velo.route_remove_child_collection", icon='X', text='Remove the current set')

        if route_settings is not None and route_settings.active_collection is not None:
            layout.label(text=iface_('Current collection: {0}').format(route_settings.active_collection.name), icon='RESTRICT_SELECT_OFF')
        if route_settings is not None:
            active_item = _ops.get_active_material_route_item(route_settings)
            if active_item is None:
                layout.label(text='Current material group: None selected', icon='MESH_DATA')
            else:
                layout.label(text=iface_('Current material group: {0}').format(_ops.describe_material_route_item(active_item)), icon='MESH_DATA')

        tree_box = layout.box()
        tree_box.label(text='Collection tree (virtual material group)', icon='OUTLINER')
        if route_settings is None or len(route_settings.route_items) == 0:
            tree_box.label(text="After clicking 'Refresh Material Groups,' the final grouping will be simulated here according to the materials", icon='INFO')
        else:
            _draw_collection_tree(tree_box.column(align=True), root, root, route_settings)

        note = layout.box()
        note.scale_y = 0.8
        note.label(text='The sets in the tree are the actual Blender sets; the leaves are the final groups simulated by material and do not represent the current actual mesh.', icon='INFO')
        note.label(text='For split single-material objects, or multi-material objects merged by the same texture, dragging in the outline will automatically restore the virtual leaf ownership.', icon='FILE_REFRESH')
        note.label(text="When clicking 'Separate by Texture and Aggregate into Collection', it will try to recombine materials with the same texture into fewer objects before aggregating.", icon='MATERIAL')
        note.label(text='First click a material group, then click the import icon on the left side of any collection row to reassign it.', icon='MOUSE_LMB')
