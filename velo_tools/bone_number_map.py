...
class VELO_WWMI_BoneMapSettings(bpy.types.PropertyGroup):
    ...
    voxel_size: bpy.props.FloatProperty(
        ...
        description='Normalized voxel size used only to order geometry candidates before complete skin-weight verification',
    )
...
_MATCHING_RESULT_VERSION = 2
_MATCHING_METHOD = "complete_skin_weights"
_MATCHING_METHOD_KEY = "_verified_matching_method"


def _matching_result_path(source_folder):
    return Path(bpy.path.abspath(str(source_folder))).resolve() / _MATCHING_RESULT_NAME


def _read_matching_result(source_folder):
    path = _matching_result_path(source_folder)
    payload = json.loads(path.read_text(encoding="utf-8"))
    if (payload.get("format") != "Velo WWMI matching result"
            or payload.get("version") != _MATCHING_RESULT_VERSION
            or payload.get("matching_method") != _MATCHING_METHOD):
        raise ValueError(iface_(
            "WWMI_MatchingResult.json was generated by an obsolete bone matcher; "
            "fill in the unpack path and regenerate it."
        ))
    return payload


def _matching_result_is_current(source_folder):
    try:
        _read_matching_result(source_folder)
...
class VELO_OT_wwmi_bone_map_generate(bpy.types.Operator):
    ...
    bl_description = 'Match unpacked .uemodel sections to WWMI Components, then prove local bone identities from every corresponding vertex and skin weight'
    ...
    def execute(self, context):
        ...
        settings[_MATCHING_METHOD_KEY] = _MATCHING_METHOD
        settings.show_original = False
        self.report({'INFO'}, iface_(
            'Generated {1} rows from complete skin-weight matching of {0} Components'
        ).format(len(evidence), len(mapping)))
        return {'FINISHED'}
...
class VELO_OT_wwmi_auto_bind(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not source_folder.strip():
            ...
            return {'CANCELLED'}
        result_current = _matching_result_is_current(source_folder)
        if not result_current and not settings.unpack_folder.strip():
...
class VELO_OT_wwmi_bone_map_load_result(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not source_folder.strip():
            ...
            return {'CANCELLED'}
        try:
            payload = _read_matching_result(source_folder)
...
class VELO_OT_wwmi_bone_map_save_result(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not settings.unpack_folder.strip():
            ...
            return {'CANCELLED'}
        if settings.get(_MATCHING_METHOD_KEY) != _MATCHING_METHOD:
            self.report({'ERROR'}, iface_(
                'Regenerate the mapping table with complete skin-weight matching before saving it.'
            ))
            return {'CANCELLED'}
        try:
            ...
            payload = {
                ...
                "version": _MATCHING_RESULT_VERSION,
