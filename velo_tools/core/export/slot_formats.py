"""Shared DXGI format-family tags for slot-style texture export."""
...
DXGI_FORMAT_NAMES = (
    ...
    "R10G10B10_XR_BIAS_A2_UNORM", "B8G8R8A8_TYPELESS",
    "B8G8R8A8_UNORM_SRGB", "B8G8R8X8_TYPELESS", "B8G8R8X8_UNORM_SRGB",
    "BC6H_TYPELESS", "BC6H_UF16", "BC6H_SF16", "BC7_TYPELESS",
...
def format_prefix(format_name: str) -> str:
    return str(format_name or "").split("_")[0]


def format_filter_index(format_name: str) -> float:
    prefix = format_prefix(format_name)
    if not prefix:
        raise ValueError("DXGI format name is empty")
    return float("83." + "".join(str(ord(char)) for char in prefix))


def emitted_format_members(format_name: str) -> tuple[str, ...]:
    if not format_name:
        return ()
    prefix = format_prefix(format_name)
...
def filter_index_text(format_name: str) -> str:
    value = format_filter_index(format_name)
    text = repr(value)
