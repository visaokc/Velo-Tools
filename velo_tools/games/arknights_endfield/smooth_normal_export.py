...
def collect_ranges(maker):
    """Opt in a component's custom ranges when any exported part has usable RG."""
    result = {}
    ...
    for index, line in enumerate(lines):
        ...
        if any_draw or opaque_call:
            ...
            if opaque_call:
                ...
                # Unknown helpers may draw or rebind geometry. Require fresh evidence.
...
def _state_preserving_calls(lines):
    ...
    while pending:
        ...
        for name, commands in pending.items():
            if any(re.match(rf"(?:draw\w*|ib|vb1|x{PARAM_INDEX})\s*=", line) for line in commands):
                continue
            calls = [match[1] for line in commands if (match := re.match(r"run\s*=\s*(.+)", line))]
