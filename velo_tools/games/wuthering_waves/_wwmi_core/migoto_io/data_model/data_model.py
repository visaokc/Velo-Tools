...
class DataModel:
    ...
    def get_mesh_data(
        ...
        if not fetch_loop_data:
            if collection != context.scene.VTWW_settings.vertex_ids_cached_collection:
                # Cache contains data for different object and must be cleared
                context.scene.VTWW_settings.vertex_ids_cache = ''
                fetch_loop_data = True
                cache_vertex_ids = True
            else:
                # Partial export is enabled
                if context.scene.VTWW_settings.vertex_ids_cache:
                    # Valid vertex ids cache exists, lets load it
                    vertex_ids_cache = numpy.array(json.loads(context.scene.VTWW_settings.vertex_ids_cache))
                else:
                    # Cache is clear, we'll have to fetch loop data once 
                    fetch_loop_data = True
                    cache_vertex_ids = True
        elif context.scene.VTWW_settings.vertex_ids_cache:
            # We're going to fetch loop data, cache must be cleared
            context.scene.VTWW_settings.vertex_ids_cache = ''
            context.scene.VTWW_settings.index_data_cache = ''
        ...
        if cache_vertex_ids:
            ...
            context.scene.VTWW_settings.vertex_ids_cache = json.dumps(vertex_ids.tolist())
            if cache_index_data:
                context.scene.VTWW_settings.index_data_cache = json.dumps(index_buffer.tolist())
            context.scene.VTWW_settings.vertex_ids_cached_collection = collection
