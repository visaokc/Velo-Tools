"""Velo import-time texture assignment for WWMI.

Ported as-is from games/arknights_endfield/import_textures.py (the 0.9.4
workflow): WWMI core extraction writes the same TextureUsage.json schema
("Component N" -> "ps-tX" -> ["hash-vs=...-ps=..."]) and names texture files
"Components-{ids} t={hash}.dds", so the parsing and the selection heuristic
(ps-t0 candidates -> ps-t10..t19 frequency -> BC7 SRGB -> largest file) apply
unchanged. Kept as a per-game copy on purpose: the heuristics may diverge per
game later. bpy is import-optional so the pure logic stays pytest-able.
"""
